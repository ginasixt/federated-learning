...
#   --root /Users/ginasixt/federated-learning/result \
#   --out  /Users/ginasixt/federated-learning/plots_out_03 \
...
def plot_scatter_recall_vs_spec(df: pd.DataFrame, outdir: Path):
    ...
    plt.legend(handles=thr_handles, title="Threshold", loc="upper right")  # Geändert von "lower right" zu "upper right"
...
def maybe_filter_idd(df: pd.DataFrame, exclude_idd: bool) -> pd.DataFrame:
    if exclude_idd and not df.empty and "alpha" in df.columns:
        return df[df["alpha"] != "idd"].copy()
    return df



def _aggregate_confusion(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate TP/TN/FP/FN per (alpha, threshold). NaNs are treated as zeros for counts."""
    needed = ["alpha", "threshold", "tp", "tn", "fp", "fn"]
    if any(col not in df.columns for col in needed):
        return pd.DataFrame(columns=["alpha","threshold","tn","fp","fn","tp","support_neg","support_pos"])

    grp = (
        df.groupby(["alpha","threshold"])[["tp","tn","fp","fn"]]
        .mean()       # Mittelwerte statt Summe
        .round()      # schön runden
        .astype(int)  # wieder als ganze Zahlen
    )

    grp = grp.reset_index()
    grp["support_neg"] = grp["tn"] + grp["fp"]
    grp["support_pos"] = grp["tp"] + grp["fn"]
    return grp


def _best_threshold_by_alpha(df: pd.DataFrame) -> pd.DataFrame:
    """Pick the best threshold per alpha by mean balanced accuracy (fallback to Youden, then recall)."""
    if "balanced_accuracy" not in df.columns:
        # fallback: use youden then recall if available
        metric_candidates = [c for c in ["youden","recall"] if c in df.columns]
        metric = metric_candidates[0] if metric_candidates else None
    else:
        metric = "balanced_accuracy"

    if metric is None:
        # choose the most frequent threshold as a crude default
        idx = df.groupby(["alpha","threshold"]).size().reset_index(name="n")
        best = idx.sort_values(["alpha","n","threshold"], ascending=[True, False, True]).drop_duplicates("alpha")
        return best[["alpha","threshold"]]

    agg = df.groupby(["alpha","threshold"])[metric].mean().reset_index()
    # higher is better
    agg["rank"] = agg.groupby("alpha")[metric].rank(ascending=False, method="first")
    best = agg[agg["rank"] == 1.0][["alpha","threshold"]]
    return best


def plot_confusion_matrices(df: pd.DataFrame, outdir: Path) -> List[Path]:
    """
    Create confusion-matrix plots (2x2) with TP, FN, TN, FP annotations for the *best* threshold per alpha.
    Files are saved as: confusion_alpha{alpha}_thr_{threshold}.png
    """
    ensure_out(outdir)
    agg_counts = _aggregate_confusion(df)
    if agg_counts.empty:
        return []

    # choose best thresholds per alpha
    best = _best_threshold_by_alpha(df)
    merged = pd.merge(best, agg_counts, on=["alpha","threshold"], how="inner")
    if merged.empty:
        return []
    ...
    for _, row in merged.iterrows():
        ...
        # dezente Matrix, keine Colorbar
        im = ax.imshow(mat, cmap="Blues", vmin=0, vmax=mat.max())

        # nur die 4 Felder mit Label + Zahl
        ...
        ax.set_title(f"Konfusionsmatrix (α={alpha}, Schwelle={thr})", fontsize=13, pad=8)

        # add a metrics footer
        footer = f"Accuracy={acc:.3f} | Recall={recall:.3f} | Spezifität={spec:.3f} | Präzision={prec:.3f}"
        fig.text(0.5, -0.02, footer, ha="center", fontsize=9)

        # --- Kontext unter die Grafik ---
        subset = df[(df["alpha"] == alpha) & (df["threshold"] == thr)]
        n_runs = int(len(subset))
        sums = subset[["tp","tn","fp","fn"]].sum(min_count=1)
        total_patients = int(sums.sum())
        pos_total = int(sums.get("tp", 0) + sums.get("fn", 0))
        neg_total = int(sums.get("tn", 0) + sums.get("fp", 0))

        # Mittelwerte der Raten zur Einordnung (woher kommt der Recall?)
        recall_mean = float(subset["recall"].mean()) if "recall" in subset else float("nan")
        spec_mean = float(subset["spec"].mean()) if "spec" in subset else float("nan")
        precision_mean = float(subset["precision"].mean()) if "precision" in subset else float("nan")

        ax.figure.text(
            0.5, -0.08,
            f"Aufsummiert über {n_runs} Runs | Insgesamt N={total_patients:,} (Pos={pos_total:,}, Neg={neg_total:,})",
            ha="center", va="top", fontsize=9
        )
        ax.figure.text(
            0.5, -0.14,
            f"Einordnung: Recall/Spezifität/Präzision oben = Mittelwert über alle Runs dieses Thresholds "
            f"(Recall̄={recall_mean:.3f}, Spezifität̄={spec_mean:.3f}, Präzision̄={precision_mean:.3f})",
            ha="center", va="top", fontsize=8
        )

        plt.tight_layout()
        outpath = outdir / f"confusion_alpha{alpha}_thr_{thr}.png"
...
def plot_roc_curves(df: pd.DataFrame, outdir: Path) -> Path:
    """Plot ROC curves (TPR vs FPR) for each alpha, with different thresholds marked."""
    ensure_out(outdir)
    ...
    for a in alpha_keys:
        sub = df[df["alpha"] == a].sort_values("threshold")
        if sub.empty or "fpr" not in sub.columns or "tpr" not in sub.columns:
            continue
        
        # Gruppiere nach Threshold und nimm Mittelwerte
        grp = sub.groupby("threshold")[["fpr", "tpr"]].mean().reset_index()
        
        label = "IID" if a == "idd" else f"α={a}"
        color = color_family_for_alpha(a)
        
        plt.plot(grp["fpr"], grp["tpr"], marker='o', label=label, color=color, linewidth=2)
        
        # Annotiere jeden Threshold-Punkt
    ...
    plt.title("ROC Curves: TPR vs FPR über Thresholds")
...
def main():
    ...
    ap.add_argument("--exclude-idd", action="store_true", help="Exclude the IDD split from all plots/aggregates")
    args = ap.parse_args()

    df = collect_runs(args.root)
    # Optional: exclude idd split
    try:
        exclude_idd = getattr(args, "exclude_idd")
    except Exception:
        exclude_idd = False
    df = maybe_filter_idd(df, exclude_idd)
    if df.empty:
    ...
    # NEW: create confusion matrices for best threshold per alpha
    for p in plot_confusion_matrices(df, args.out):
        paths.append(p)
    p6 = plot_roc_curves(df, args.out)
    if p6: paths.append(p6)
