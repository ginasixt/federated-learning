...
def _aggregate_confusion(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate TP/TN/FP/FN per (alpha, threshold) using mean to avoid run-count inflation."""
    ...
    grp = (
        df.groupby(["alpha","threshold"], dropna=False)[["tp","tn","fp","fn"]]
...
def plot_confusion_matrices(df: pd.DataFrame, outdir: Path) -> List[Path]:
    ...
    Adds context lines: number of runs, summed N, and formulas for metrics.
    If `population` is set, counts are reconstructed from averaged rates (recall/spec/prevalence).
    """
    ensure_out(outdir)
    ...
    for _, row in merged.iterrows():
        ...
        # If a target population is provided AND rates exist, rebuild counts from averaged rates
        if population is not None and all(c in df.columns for c in ["recall","spec","prevalence"]):
            rate_row = (
                df[(df["alpha"] == alpha) & (df["threshold"] == thr)]
                [["recall","spec","prevalence"]]
                .mean()
            )
            TPR = float(rate_row.get("recall", float("nan")))
            TNR = float(rate_row.get("spec", float("nan")))
            prev = float(rate_row.get("prevalence", float("nan")))
            P = int(round(prev * population))
            Nn = int(round((1 - prev) * population))
            TP = int(round(TPR * P))
            FN = P - TP
            TN = int(round(TNR * Nn))
            FP = Nn - TN
        ...
        ax.set_xticks([0,1], labels=["Vorhersage: Negativ", "Vorhersage: Positiv"], rotation=15, ha="right")
        ax.set_yticks([0,1], labels=["Tatsache: Negativ", "Tatsache: Positiv"])
        ax.set_title(f"Konfusionsmatrix  (alpha={alpha}, thr={thr})")

        footer = f"Accuracy={acc:.3f} | Recall (TPR)={recall:.3f} | Spezifität={spec:.3f} | Präzision={prec:.3f}"
        ax.figure.text(0.5, -0.02, footer, ha="center", va="top", fontsize=9)

        # Zusatzzeile 1: Runs & aufsummierte Gesamtfälle (über Runs)
        ...
        try:
            ...
            info1 = f"Aufsummiert über {n_runs} Runs | Insgesamt N={total_patients:,} (Pos={pos_total:,}, Neg={neg_total:,})"
        except Exception:
            info1 = f"Aufsummiert über {n_runs} Runs"

        # Zusatzzeile 2: Formeln
        info2 = "Berechnung: Recall=TP/(TP+FN), Spezifität=TN/(TN+FP), Präzision=TP/(TP+FP), Accuracy=(TP+TN)/(TP+TN+FP+FN)"
        ax.figure.text(0.5, -0.08, info1, ha="center", va="top", fontsize=9)
        ax.figure.text(0.5, -0.14, info2, ha="center", va="top", fontsize=8)
...
def main():
    ...
    for p in plot_confusion_matrices(df, args.out, args.population):
        paths.append(p)
